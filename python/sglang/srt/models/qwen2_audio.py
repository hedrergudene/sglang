...
import logging
from typing import Any, Dict, Iterable, List, Mapping, Optional, Set, Tuple, TypedDict, Union

import torch
from torch import nn
from transformers import BatchFeature, Qwen2AudioConfig, Qwen2AudioEncoder, Qwen2AudioProcessor
...
class Qwen2AudioProcessingInfo:
    def get_hf_config(self):
        return self.ctx.get_hf_config(Qwen2AudioConfig)

    def get_hf_processor(self, *, sampling_rate: Optional[int] = None, **kwargs: object) -> Qwen2AudioProcessor:
        return self.ctx.get_hf_processor(Qwen2AudioProcessor, **kwargs)

    def get_feature_extractor(self, *, sampling_rate: Optional[int] = None) -> WhisperFeatureExtractor:
...
class Qwen2AudioDummyInputsBuilder:
    def get_dummy_processor_inputs(self, seq_len: int, mm_counts: Mapping[str, int]) -> ProcessorInputs:
        ...
        return ProcessorInputs(prompt_text="<|AUDIO|>" * num_audios, mm_data=mm_data)
...
class Qwen2AudioMultiModalProcessor:
    ...
    def _call_hf_processor(self, prompt: str, mm_data: Mapping[str, object], mm_kwargs: Mapping[str, Any]) -> BatchFeature:
        ...
        return super()._call_hf_processor(prompt=prompt, mm_data=mm_data, mm_kwargs=mm_kwargs)
    ...
    def _get_prompt_replacements(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, object], out_mm_kwargs: MultiModalKwargs) -> list[PromptReplacement]:
        ...
        def get_replacement_qwen2_audio(item_idx: int):
            ...
            return PromptReplacementDetails(full=[audio_bos_id] + audio_tokens + [audio_eos_id], features=audio_tokens)

        return [PromptReplacement(modality="audio", target=audio_token, replacement=get_replacement_qwen2_audio)]

class Qwen2AudioForConditionalGeneration(nn.Module):
    def __init__(self, config: Qwen2AudioConfig, quant_config: Optional[QuantizationConfig] = None):
        super().__init__()
        self.config = config
        ...
        self.language_model = self._init_language_model(config, quant_config)

    def _init_language_model(self, config: Qwen2AudioConfig, quant_config: Optional[QuantizationConfig]):
        return LlamaModel(config.text_config, quant_config=quant_config)
    ...
    def _parse_and_validate_audio_input(self, **kwargs: object) -> Optional[Qwen2AudioInputs]:
        ...
        if not isinstance(input_features, (torch.Tensor, list)):
            raise ValueError("Incorrect type of audio input features. Got type: {type(input_features)}")
        return Qwen2AudioInputs(input_features=input_features, feature_attention_mask=feature_attention_mask)
    ...
    def _process_audio_input(self, audio_input: Qwen2AudioInputs) -> torch.Tensor:
        ...
        audio_features_mask = torch.arange(max_audio_tokens).expand(num_audios, max_audio_tokens).to(audio_output_lengths.device) < audio_output_lengths
    ...
    def get_input_embeddings(self, input_ids: torch.Tensor, multimodal_embeddings: Optional[NestedTensors] = None) -> torch.Tensor:
        ...
        if multimodal_embeddings is not None:
            inputs_embeds = merge_multimodal_embeddings(input_ids, inputs_embeds, multimodal_embeddings, self.config.audio_token_index)
        return inputs_embeds

    def forward(self, input_ids: torch.Tensor, positions: torch.Tensor, forward_batch: ForwardBatch, intermediate_tensors: Optional[IntermediateTensors] = None, inputs_embeds: Optional[torch.Tensor] = None, **kwargs: object) -> Union[torch.Tensor, IntermediateTensors]:
        if intermediate_tensors is not None:
            ...
            input_ids = None
        hidden_states = self.language_model(input_ids, positions, forward_batch, inputs_embeds=inputs_embeds)
        return hidden_states
